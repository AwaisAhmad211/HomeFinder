...
export function StoreHome() {
  ...
  const safeBanners = (data?.banners ?? []).filter((item) => !!item?.imageUrl);
  const safeCategories = (data?.categories ?? []).filter(
    (item) => !!item?._id && !!item?.slug,
  );
  const safeCoupons = (data?.coupons ?? []).filter((item) => !!item?._id);
  const safeProducts = (data?.recentProducts ?? []).filter(
    (item) => !!item?._id && !!item?.slug,
  );

  const mainBanner = safeBanners[0] || null;
  const sideBanners = safeBanners.slice(1, 3);
          ...
          {!!safeCategories.length ? (
            <section>
              ...
              <div className={categoryGridClass}>
                {safeCategories.slice(0, 4).map((categoryItem) => (
                  <Link
                    key={categoryItem._id}
                    to={`/collections?category=${categoryItem.slug}`}
                  >
          ...
          {!!safeCoupons.length ? (
            <section>
              ...
              <div className={couponGridClass}>
                {safeCoupons.slice(0, 4).map((coupon) => (
            ...
            <section>
              ...
              <div className={productGridClass}>
                {safeProducts.slice(0, 4).map((product) => (
                  <Link to={`/collection/${product.slug}`} key={product._id}>
